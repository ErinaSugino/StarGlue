...
MainInterface::MainInterface(UniverseClientPtr client, WorldPainterPtr painter, CinematicPtr cinematicOverlay) {
  ...
  if (m_config->hasCustom1) {
      m_customPane1 = make_shared<ScriptPane>(m_client, m_config->custom1PaneConfig);
      m_paneManager.registerPane(MainInterfacePanes::CustomButton1, PaneLayer::Window, m_customPane1);
  }
  if (m_config->hasCustom2) {
      m_customPane2 = make_shared<ScriptPane>(m_client, m_config->custom2PaneConfig);
      m_paneManager.registerPane(MainInterfacePanes::CustomButton2, PaneLayer::Window, m_customPane2);
  }
  if (m_config->hasCustom3) {
      m_customPane3 = make_shared<ScriptPane>(m_client, m_config->custom3PaneConfig);
      m_paneManager.registerPane(MainInterfacePanes::CustomButton3, PaneLayer::Window, m_customPane3);
  }
...
PanePtr MainInterface::createEscapeDialog() {
  ...
  escapeDialog->fetchChild<LabelWidget>("lblversion")->setText(strf("Starbound - %s (%s) - StarGlue - %s", StarVersionString, StarArchitectureString, StarGlueVersionString));
  return escapeDialog;
}
...
void MainInterface::renderMainBar() {
  ...
  if (m_config->hasCustom1) {
      Vec2I custom1ButtonPos = barPos + m_config->mainBarCustom1ButtonOffset * interfaceScale();
      drawStateButton(MainInterfacePanes::CustomButton1,
          custom1ButtonPos,
          m_config->mainBarCustom1ButtonPoly,
          m_config->custom1Image,
          m_config->custom1ImageHover,
          m_config->custom1ImageOpen,
          m_config->custom1ImageHoverOpen,
          assets->json("/interface.config:mainBar.custom[0].tooltip").toString());
  }
  if (m_config->hasCustom2) {
      Vec2I custom2ButtonPos = barPos + m_config->mainBarCustom2ButtonOffset * interfaceScale();
      drawStateButton(MainInterfacePanes::CustomButton2,
          custom2ButtonPos,
          m_config->mainBarCustom2ButtonPoly,
          m_config->custom2Image,
          m_config->custom2ImageHover,
          m_config->custom2ImageOpen,
          m_config->custom2ImageHoverOpen,
          assets->json("/interface.config:mainBar.custom[1].tooltip").toString());
  }
  if (m_config->hasCustom3) {
      Vec2I custom3ButtonPos = barPos + m_config->mainBarCustom3ButtonOffset * interfaceScale();
      drawStateButton(MainInterfacePanes::CustomButton3,
          custom3ButtonPos,
          m_config->mainBarCustom3ButtonPoly,
          m_config->custom3Image,
          m_config->custom3ImageHover,
          m_config->custom3ImageOpen,
          m_config->custom3ImageHoverOpen,
          assets->json("/interface.config:mainBar.custom[2].tooltip").toString());
  }
}
...
void MainInterface::overlayClick(Vec2I const& mousePos, MouseButton mouseButton) {
  ...
  if (m_config->hasCustom1 && overButton(m_config->mainBarCustom1ButtonPoly, mousePos)) {
      m_paneManager.toggleRegisteredPane(MainInterfacePanes::CustomButton1);
      return;
  }
  if (m_config->hasCustom2 && overButton(m_config->mainBarCustom2ButtonPoly, mousePos)) {
      m_paneManager.toggleRegisteredPane(MainInterfacePanes::CustomButton2);
      return;
  }
  if (m_config->hasCustom3 && overButton(m_config->mainBarCustom3ButtonPoly, mousePos)) {
      m_paneManager.toggleRegisteredPane(MainInterfacePanes::CustomButton3);
      return;
  }
